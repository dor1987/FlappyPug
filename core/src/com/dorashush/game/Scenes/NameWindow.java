        ...
        submitBtn.addListener(new ClickListener() {
                ...
                public void clicked(InputEvent event, float x, float y) {
                    ...
                    Runnable closeNameWindow = new Runnable() {
                        @Override
                        public void run() {
                            screen.nameWindowControl(false);
                            dispose();
                        }
                    };
                    stage.addAction(sequence(parallel( scaleTo(.01f, .01f, 1f, Interpolation.pow5)),run(closeNameWindow)));
